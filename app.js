...
function cardHTML(c,rev=false){return`<div class="card ${rev?"reversed":""}"><div class="card-inner"><img src="${c.image}" alt="${c.name}牌面"><div class="fallback" hidden>${c.symbol}</div></div></div>`}
...
function buildReading(){const q=currentQuestion(),ctx=q?`关于「${q}」`:"关于你此刻最在意的事情",guide=categoryWords[readingCategory()];if(spread==="single"){const d=drawn[0],c=deck[d.id],tone=isBright(d,c);return`${ctx}，${c.name}以${d.reversed?"逆":"正"}位出现。${d.reversed?c.rev:c.up}\n\n${tone?guide.bright:guide.hard}\n\n${tone?guide.summaryGood:guide.summaryHard}`};const parts=drawn.map(cardParagraph),bright=drawn.filter(d=>isBright(d,deck[d.id])).length;return`${parts.join("\n\n")}\n\n${bright>=2?guide.summaryGood:guide.summaryHard}`}
